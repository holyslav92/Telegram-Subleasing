...
Оформляете бронь на <b><a href="https://добрыйдом-72.рф/">нашем официальном сайте</a></b> по <b>прямым ценам</b> или через <b><a href="https://t.me/Dobriy_dom_Tyumen">менеджера</a></b>, получаете понятную инструкцию с кодом доступа и заезжаете благодаря <b>бесконтактному заезду 24/7</b> в любое время суток за 5 минут.

1️⃣ <b>Чистота и уют</b>: отельное постельное белье, свежие полотенца и индивидуальные гигиенические наборы.
2️⃣ <b>Полное оснащение</b>: скоростной Wi-Fi, фен, утюг, стиральная машина и чайная зона.
3️⃣ <b>Командировочным</b>: предоставляем кассовый чек с QR-кодом и полный пакет отчетных документов.
...
Все актуальные цены и свободные даты без посредников и комиссий представлены на <b><a href="https://добрыйдом-72.рф/">нашем официальном сайте</a></b>. 
...
2️⃣ <b>Экскурсии по городу и окрестностям</b> — тематические пешеходные и автобусные маршруты от наших партнеров собраны на <b><a href="https://добрыйдом-72.рф/excursions/">странице туров</a></b>.
3️⃣ <b>Сибирская кухня</b> — строганина, блюда из дичи и сибирские десерты в заведениях исторического центра.

Для комфортного проживания выбирайте любую из 60+ квартир «Доброго дома». <b>Прямые цены</b> без наценок и <b>бесконтактный заезд 24/7</b> всегда ждут вас на <b><a href="https://добрыйдом-72.рф/">нашем официальном сайте</a></b>."""
...
def main():
    ...
    parser.add_argument("--chat", default="", help="Целевой чат/группа")
    parser.add_argument("--token", default="", help="Токен бота")

    args = parser.parse_args()

    token = args.token or os.environ.get("TELEGRAM_BOT_TOKEN", "").strip() or load_tenant_config().get("telegram_bot", {}).get("bot_token", "")
    chat = args.chat or os.environ.get("TELEGRAM_CHAT_ID", "").strip() or load_tenant_config().get("telegram_bot", {}).get("target_chat", "")
    ...
    if args.send:
        print(f"Отправка единого поста в Telegram чат {chat} (фото: {args.photo_file or photo_url or 'нет'}, бесшумный режим: {args.silent})...")
        try:
