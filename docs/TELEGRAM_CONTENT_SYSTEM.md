...
1. **Генерация фото, Pexels API и Image-to-Image:** Для точного воспроизведения логотипа без искажения геометрии модель GPT Image 2 переведена в режим **Image-to-Image** с передачей публичного URL референса логотипа (`input_urls`). Если настроен FTP сайта, логотип автоматически загружается на сервер сайта `https://добрыйдом-72.рф/uploads/`.
