...
1. **Генерация фото, Pexels API и Image-to-Image:** 
   ...
   - В `input_urls` передается:
     а) Эталонный логотип бренда «Добрый дом» напрямую с официального сайта (`https://добрыйдом-72.рф/wp-content/uploads/2026/04/dizajn-bez-nazvaniya-36.png`);
     б) Прямая ссылка на реальное профессиональное фото из Pexels API (находится динамически под тему поста).
   - Модель перенимает композицию, цветовую гамму и естественный свет из Pexels-референса, интегрирует оригинальный логотип без геометрических искажений и выводит стильный кириллический заголовок.
2. **Ротация рубрик и защита от повторов:** 
   ...
   - Среда: Заметки радушного хозяина (живой диалог от первого лица, истории уюта)
   - Четверг: Сервис, стандарты чистоты и бесконтактный заезд 24/7
   - Пятница: Афиша выходного дня и культурные события
   - Суббота: Спецпредложения и скидки (раннее и длительное бронирование)
   - Воскресенье: Гид по термам, горячим источникам и прогулкам по Тюмени
