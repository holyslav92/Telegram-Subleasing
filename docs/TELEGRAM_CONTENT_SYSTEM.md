...
1. **Генерация фото и Pexels API:** Идеи для обложек обогащаются реальными композициями через Pexels API для достижения аутентичной фотореалистичности без "AI-пластика". Выполняется до 3 попыток генерации через GRSAI с интервалом. Если изображение не удалось сгенерировать — пост не публикуется, одиночный логотип вместо фото не высылается.
