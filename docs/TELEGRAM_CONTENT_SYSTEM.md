...
1. **Генерация фото, Pexels API и Image-to-Image:** 
   ...
   - В `input_urls` передается:
     а) Эталонный логотип бренда «Добрый дом» (через FTP сайта или защищенный веб-URL);
...
2. **Ротация рубрик и виральность:** Тексты пишутся живо, увлекательно, без шаблонов, чередуются гиды, афиша, обзоры ЖК и душевные заметки радушного хозяина («Заметки хозяина» от первого лица). Пост должен вызывать желание поставить реакцию или переслать друзьям.
