...
- Тональность (Tone of Voice): заботливый, современный сервис посуточной аренды в Тюмени. Квартиры берутся в бережное доверительное управление, профессионально готовятся и сдаются с отельным уровнем чистоты и уюта. Мы не пишем «мы хозяева квартир» или «заметки хозяина». Акцент на заботе, стандартах комфорта, отельном сатине, бесконтактном заезде 24/7 и отчетных документах командировочным.
...
- Никаких шаблонных текстов — живой, кинематографичный слог, чередование гидов со статьями о стандартах уюта («Уют в деталях»).
- Дистрибуция:
...
- Обложка поста генерируется через GRSAI API (модель GPT Image 2 в режиме Image-to-Image) в формате 1:1.
- В `input_urls` всегда передаются ровно 2 актуальных референса (старые не накапливаются):
  1. Эталонный логотип компании: https://добрыйдом-72.рф/wp-content/uploads/2026/04/dizajn-bez-nazvaniya-36.png
  2. Реальное фото из Pexels API или кураторский снимок конкретной локации под тему текущего дня (без небоскребов, мегаполисов и чужих высоток).
- Работает режим `reference-first`: второе изображение является исходником композиции и геометрии. Модель не имеет права выдумывать мосты, здания, панорамы и другие объекты, которых нет на фото.
- Для локационных тем без кураторского референса генерация пропускает тему; генерация только из текстового описания запрещена. Конфигурация: `shared/telegram-visual-rules.json`.
- На изображении обязателен крупный, стильный, контрастный кириллический заголовок в белом цвете современным шрифтом.
- **Логотип — только копия референса:** reference image 1 — готовый PNG. Модель должна **вклеить** его как есть (composite/copy), без перерисовки. Не описывать логотип словами в промпте.
