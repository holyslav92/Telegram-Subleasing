...
- В `input_urls` всегда передаются ровно 2 актуальных референса (старые не накапливаются):
  ...
  2. Новое свежее фото-референс из Pexels API под конкретную тему текущего дня (без небоскребов, мегаполисов и чужих высоток — только уютный интерьер или аутентичная Тюмень).
