...
- В качестве исходного эталонного референса логотипа передается массив `input_urls` с официальным медиа-файлом логотипа: https://добрыйдом-72.рф/wp-content/uploads/2026/04/dizajn-bez-nazvaniya-36.png
- В `input_urls` также передается реальное фото-референс из Pexels API для заимствования аутентичной композиции, света и ракурса.
- На изображении обязателен крупный, стильный, контрастный кириллический заголовок современным шрифтом (например, 'УЮТ В КАЖДОЙ ДЕТАЛИ', 'БЕСКОНТАКТНЫЙ ЗАЕЗД 24/7') для привлечения внимания в ленте.
- Логотип бренда (знак зеленых шторок с красным цветком и четкая надпись «Добрый дом») интегрируется на аккуратной чистой белой плашке или в верхнем углу без искажения пропорций и фирменных цветов.
